...
-- Main automation loop for races
local function autoRaceLoop()
    while autoRaceActive do
        pcall(function()
            checkAndRespawnCar() -- Check before the loop
            debugPrint("Looking for a race...")
            -- 1. Get Race Name
            local eventNameLabel = player.PlayerGui:WaitForChild("LiveQueue"):WaitForChild("Center"):WaitForChild("Status"):WaitForChild("EventName")
            local eventName = eventNameLabel.Text
            
            if eventName == "" or eventName == "Intermission" then
                debugPrint("Not in a race queue. Waiting...")
                task.wait(1)
                return
            end

            local trimmedName = eventName:gsub("%s+", "")
            local raceName = raceNameMap[trimmedName] or trimmedName
            debugPrint("Found race event: '" .. eventName .. "', using folder name: '" .. raceName .. "'")

            if raceName == previousRaceName then
                debugPrint("Same as previous race, waiting to avoid issues...")
                task.wait(1)
                return
            end

            debugPrint("DEBUG: Checking for workspace.Races...")
            if not workspace:FindFirstChild("Races") then
                debugPrint("ERROR: The 'Races' folder does not exist in workspace.")
                task.wait(1)
                return
            end
            debugPrint("DEBUG: workspace.Races folder found.")

            -- 2. Find race folder and teleport
            local raceFolder = workspace.Races:FindFirstChild(raceName)
            if not raceFolder then
                debugPrint("ERROR: Could not find a race folder named '" .. raceName .. "' inside workspace.Races. Waiting 2s...")
                task.wait(2)
                return
            end
            debugPrint("DEBUG: Race folder '" .. raceName .. "' found.")

            local queuePart = raceFolder:FindFirstChild("MultiplayerQueue")
            if not queuePart then
                debugPrint("ERROR: Found race folder '".. raceName .."', but could not find 'MultiplayerQueue' inside it. Waiting 2s...")
                task.wait(2)
                return
            end
            debugPrint("DEBUG: MultiplayerQueue part found.")

            local teleportPos = queuePart:GetPivot().Position
            debugPrint("Teleporting to race start...")
            if not TptoVector(teleportPos) then
                debugPrint("ERROR: The TptoVector function failed.")
                task.wait(2)
                return
            end
            local car = getCar()
            ...
            local wasAnchored = root.Anchored
            root.Anchored = true
            task.wait(2) -- Wait a moment for world to load
            root.Anchored = wasAnchored
            stopCarVelocity()
            debugPrint("Waiting for race to start... (30s)")
            task.wait(28)
            if not autoRaceActive then return end
            -- 3. Run Auto Win
            autoWin(raceFolder)
            task.wait(.5)
            autoWin(raceFolder,true)
            -- Stop the car completely after the race
            toggleNoclip(false)
            stopCarVelocity()
            
            -- 4. Claim Rewards            
            debugPrint("Race finished. Waiting for results screen...")
            task.wait(.5)
            -- Wait for the Claim button to appear
            local claimButton = player.PlayerGui:WaitForChild("RaceResults"):WaitForChild("Frame"):WaitForChild("Footer"):WaitForChild("ClaimButton")
            local closeButton = player.PlayerGui:WaitForChild("RaceResults"):WaitForChild("Frame"):WaitForChild("Footer"):WaitForChild("CloseButton")

            if claimButton.Visible then
                debugPrint("Clicking ClaimButton...")
                game:GetService("ReplicatedStorage"):WaitForChild("Systems"):WaitForChild("LiveQueue"):WaitForChild("BankPayout"):FireServer()
                clickButton(claimButton)
            end
            
            if closeButton.Visible then
                debugPrint("Clicking CloseButton...")
                game:GetService("ReplicatedStorage"):WaitForChild("Systems"):WaitForChild("LiveQueue"):WaitForChild("BankPayout"):FireServer()
                clickButton(closeButton)
            end

            previousRaceName = raceName
        end)
        checkAndRespawnCar() -- Check after the loop
        task.wait(1) -- Cooldown in case of errors
    end
...
tab:CreateToggle("Auto Server Race", false, function(val)
    autoRaceActive = val
    if autoRaceActive then
        previousRaceName = nil -- Reset on new activation
        task.spawn(autoRaceLoop)
    else
        previousRaceName = nil -- Also reset on deactivation
    end
end)
    ...
    while customAutoRaceActive do
        pcall(function()
            ...
            if claimButton and claimButton.Visible then
                debugPrint("Custom Race Sequence: Clicking ClaimButton...")
                game:GetService("ReplicatedStorage"):WaitForChild("Systems"):WaitForChild("LiveQueue"):WaitForChild("BankPayout"):FireServer()
                clickButton(claimButton)
            end
            
            if closeButton and closeButton.Visible then
                debugPrint("Custom Race Sequence: Clicking CloseButton...")
                game:GetService("ReplicatedStorage"):WaitForChild("Systems"):WaitForChild("LiveQueue"):WaitForChild("BankPayout"):FireServer()
                clickButton(closeButton)
            end
            
            debugPrint("Custom Race Sequence: Finished race. Waiting 1s before starting next one.")
            toggleNoclip(false)
            task.wait(.1)
...
tab:CreateButton("Manual Auto Win", function()
    task.spawn(function()
        debugPrint("Manual Auto Win: Scanning for active race...")
        local racesFolder = workspace:FindFirstChild("Races")
        if not racesFolder then
            debugPrint("Manual Auto Win: ERROR: The 'Races' folder does not exist in workspace.")
            return
        end
        ...
        local foundRaceFolder = nil

        for _, raceFolder in ipairs(racesFolder:GetChildren()) do
            if raceFolder:IsA("Folder") then
                local checkpointsFolder = raceFolder:FindFirstChild("Checkpoints")
                if checkpointsFolder then
                    for _, descendant in ipairs(checkpointsFolder:GetDescendants()) do
                        if descendant:IsA("BasePart") and targetColors[descendant.BrickColor.Name] then
                            foundRaceFolder = raceFolder
                            break
                        end
                    end
                end
            end
            if foundRaceFolder then
                break
            end
        end

        if foundRaceFolder then
            debugPrint("Manual Auto Win: Found active race: " .. foundRaceFolder.Name .. ". Starting autoWin...")
            
            local wasFarming = autoRaceActive
            autoRaceActive = true
            
            autoWin(foundRaceFolder)
            
            autoRaceActive = wasFarming
            
            debugPrint("Manual Auto Win: Finished.")
        else
            debugPrint("Manual Auto Win: No active race found.")
        end
    end)
end)
